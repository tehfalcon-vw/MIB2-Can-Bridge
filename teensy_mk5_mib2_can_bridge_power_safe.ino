...
static constexpr const char *FIRMWARE_VERSION = "MK5-POWER-SAFE-RC1-2026-07-25";
...
static constexpr uint32_t EEPROM_MAGIC = 0x4D494238;  // "MIB8" profile with power-safe defaults
...
static constexpr int EEPROM_ADDR_POWER_COMPAT = EEPROM_ADDR_NAV_ARROW_HINTS + sizeof(uint8_t);
static constexpr int EEPROM_ADDR_DEEP_SLEEP = EEPROM_ADDR_POWER_COMPAT + sizeof(uint8_t);
...
static constexpr uint32_t DDP_SLEEP_IDLE_TIMEOUT_MS = 10000;
static constexpr uint32_t DDP_WAKE_BURST_GAP_MS = 500;
...
static constexpr uint8_t DDP_WAKE_MIN_BURST_FRAMES = 1;
...
static constexpr bool DEFAULT_POWER_COMPAT_ENABLED = true;
static constexpr bool DEFAULT_DEEP_SLEEP_ENABLED = true;
...
static bool powerCompatEnabled = DEFAULT_POWER_COMPAT_ENABLED;
static bool deepSleepEnabled = DEFAULT_DEEP_SLEEP_ENABLED;
...
static uint32_t powerCompatConversions = 0;
static uint32_t suppressedLocalTxFrames = 0;
...
void enterActivePowerState() {
  ...
  if (ddpBootEnabled && !ddpEnabled && !ddpBootStartPending) {
...
void enterShutdownGrace() {
  ...
  if (ddpVerbose || verboseLogging) {
    Serial.println("Power: run activity stopped; DDP off, starting radio shutdown grace");
  }
}
...
void enterDeepPowerSleep() {
  ...
  lastNonDdpCanActivityMs = 0;
  vehicleWakeBurstStartedMs = 0;
  lastVehicleWakeFrameMs = 0;
  vehicleWakeBurstFrames = 0;

  // SN65HVD230 standby disables both drivers. The vehicle receiver remains
  // active and its RXD falling edge wakes the Teensy on CRX1/pin 23.
  ...
  // Keep the radio-side driver disabled until 0x2C3/0x575 proves a real wake.
...
bool isVehicleWakeActivityFrame(const CAN_message_t &msg) {
  ...
  return msg.id == RUN_ACTIVITY_CAN_ID_2C3 || msg.id == RUN_ACTIVITY_CAN_ID_575;
}

void noteVehicleWakeActivity(const CAN_message_t &msg) {
  const uint32_t now = millis();
  lastNonDdpCanActivityMs = now;
  if (msg.id == RUN_ACTIVITY_CAN_ID_2C3) {
    runActivity2c3Frames++;
  } else if (msg.id == RUN_ACTIVITY_CAN_ID_575) {
...
bool forwardRadioPowerCompatibility(const CAN_message_t &received) {
  if (!powerCompatEnabled || received.flags.extended || received.len != 8 ||
      (received.id != POWER_COMPAT_CAN_ID_436 && received.id != POWER_COMPAT_CAN_ID_439) ||
      received.buf[0] != 0x01 || received.buf[5] != 0x06) {
    return false;
  }

  CAN_message_t normalized = received;
  normalized.len = 6;
  canVehicle.write(normalized);

  if (received.id == POWER_COMPAT_CAN_ID_436) {
    normalized.id = POWER_COMPAT_CAN_ID_439;
    canVehicle.write(normalized);
  }

  powerCompatConversions++;
  if (verboseLogging) {
    printFrame("power normalized", received);
  }
  return true;
}
...
void printStatus() {
  ...
  Serial.print("  powerCompatEnabled/conversions=");
  Serial.print(powerCompatEnabled ? "on/" : "off/");
  Serial.println(powerCompatConversions);
  Serial.print("  suppressedLocalTxFrames=");
  Serial.println(suppressedLocalTxFrames);
  Serial.print("  deepSleepEnabled=");
  Serial.println(deepSleepEnabled ? "on" : "off");
  Serial.print("  runActivity 0x2C3/0x575=");
  Serial.print(runActivity2c3Frames);
  Serial.print("/");
  Serial.println(runActivity575Frames);
  Serial.print("  shutdownGrace/deepSleep/wakeups=");
  ...
  Serial.print("  lastRunActivityMsAgo=");
  if (lastNonDdpCanActivityMs == 0) {
    Serial.println("n/a");
  } else {
...
void saveSettings() {
  ...
  EEPROM.put(EEPROM_ADDR_POWER_COMPAT, static_cast<uint8_t>(powerCompatEnabled ? 1 : 0));
  EEPROM.put(EEPROM_ADDR_DEEP_SLEEP, static_cast<uint8_t>(deepSleepEnabled ? 1 : 0));
}
...
void loadSettings() {
  ...
  if (storedMagic != EEPROM_MAGIC) {
    ...
    powerCompatEnabled = DEFAULT_POWER_COMPAT_ENABLED;
    deepSleepEnabled = DEFAULT_DEEP_SLEEP_ENABLED;
    saveSettings();
    return;
  }
  ...
  uint8_t storedPowerCompat = 0xFF;
  EEPROM.get(EEPROM_ADDR_POWER_COMPAT, storedPowerCompat);
  powerCompatEnabled = boolFromStoredValue(storedPowerCompat, DEFAULT_POWER_COMPAT_ENABLED);

  uint8_t storedDeepSleep = 0xFF;
  EEPROM.get(EEPROM_ADDR_DEEP_SLEEP, storedDeepSleep);
  deepSleepEnabled = boolFromStoredValue(storedDeepSleep, DEFAULT_DEEP_SLEEP_ENABLED);
...
void printHelp() {
  ...
  Serial.println("  power status - print power and bridge state");
  Serial.println("  power compat on/off - persist 0x436/0x439 shutdown compatibility");
  Serial.println("  power sleep on/off  - persist Teensy deep sleep");
  Serial.println("  power active - force a temporary active bridge for bench testing");
...
void resetCounters() {
  ...
  powerCompatConversions = 0;
  suppressedLocalTxFrames = 0;
...
void processCommand(char *line) {
  ...
  if (strcmp(cmd, "power") == 0) {
    ...
    if (strcmp(subcommand, "compat") == 0 || strcmp(subcommand, "sleep") == 0) {
      char *value = strtok(nullptr, " \t");
      if (value == nullptr || (strcmp(value, "on") != 0 && strcmp(value, "off") != 0)) {
        Serial.println("Use: power compat on/off or power sleep on/off");
        return;
      }

      const bool enabled = strcmp(value, "on") == 0;
      if (strcmp(subcommand, "compat") == 0) {
        powerCompatEnabled = enabled;
        Serial.print("powerCompatEnabled=");
      } else {
    ...
    if (strcmp(subcommand, "active") == 0) {
      ...
      enterActivePowerState();
      Serial.println("Bridge forced active; normal run-activity timeout still applies");
      return;
    }

    Serial.println("Unknown power command. Use: power status/compat/sleep/active");
    return;
  }
...
void handleRadioToVehicle() {
  ...
  while (canRadio.read(msg)) {
    ...
    if (!forwardRadioPowerCompatibility(msg)) {
      canVehicle.write(msg);
    }
  }
}
